...
func registerGuardian(vault *mudra.KramaVault) {
	...
	if err != nil {
		common.Err(errors.Wrap(err, "failed to create moi-client"))
	}

	if isGuardianRegistered(vault.KramaID(), client) {
		common.Err(errors.New("Guardian already registered"))
	}

	moiID, err := vault.MOiID()
	if err != nil {
		common.Err(errors.Wrap(err, "failed to generate moiID"))
	}
	...
	moiIDpublicKey, err := vault.GetPublicKeyAt(common2.DefaultMOIIDPath)
	if err != nil {
		common.Err(err)
	}

	log.Println(types.HexToAddress(moiID).Bytes(), moiIDpublicKey) // FIXME: kramaID.moiID is not returning the corr
...
func isGuardianRegistered(kramaID id.KramaID, client *moiclient.Client) bool {
	...
	if err != nil {
		common.Err(errors.Wrap(err, "failed to fetch guardian information from the network"))
	}
