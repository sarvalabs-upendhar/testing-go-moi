...
func TestFetchICSNodeSet(t *testing.T) {
	...
	getTesseractParamsWithContextLock := func(address common.Address, ctxHash common.Hash) *createTesseractParams {
		return &createTesseractParams{
			address: address,
			headerCallback: func(header *common.TesseractHeader) {
				setContextLock(header, address, getContextLockInfo(ctxHash, common.NilHash, 0))
			},
		}
	}
	...
	for _, test := range testcases {
		t.Run(test.name, func(t *testing.T) {
			...
			checkIfICSNodeSetMatches(
				t,
				ics,
				common.NewNodeSet(test.behNodes, test.behPK),
				common.NewNodeSet(test.randNodes, test.randPK),
				common.NewNodeSet(test.randSet, test.randsetPK),
			)
		})
	}
}
