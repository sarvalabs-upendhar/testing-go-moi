...
func (t *TesseractGrid) TesseractsCopy() []*types.Tesseract {
	grid := make([]*types.Tesseract, 0, len(t.Tesseracts))
	for _, v := range t.Tesseracts {
		grid = append(grid, types.NewTesseract(v.Header(), v.Body(), v.Interactions(), v.Receipts(), v.Seal(), v.Sealer()))
	}
