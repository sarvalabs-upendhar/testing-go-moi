...
func InvokerState(invoker *guna.StateObject) LogicInvokeOption {
...
func InvokeLogic(logicID types.LogicID, state *guna.StateObject, opts ...LogicInvokeOption) (
	...
	// Create an IxnObject
	ixn := engineio.NewIxnObject(types.IxLogicInvoke, invoker.callsite, invoker.calldata)
	...
	if !result.Ok() {
		return invoker.fueltank.Consumed, &types.LogicInvokeReceipt{Error: result.Error}, nil
	}

	// Return the total fuel consumed and the return data
	return invoker.fueltank.Consumed, &types.LogicInvokeReceipt{Outputs: result.Outputs}, nil
}
