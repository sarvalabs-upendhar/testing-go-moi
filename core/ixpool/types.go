...
type promoteRequest struct {
	account map[ktypes.Address]interface{}
}

type enqueueRequest struct {
	ix ktypes.Interactions
}
...
func (m *accountsMap) initOnce(addr ktypes.Address, nonce uint64) *account {
...
func (m *accountsMap) exists(addr ktypes.Address) bool {
	_, ok := m.Load(addr)
...
func (m *accountsMap) getWaitPrimaries() *maxWaitQueue {
	...
	m.Range(func(key, value interface{}) bool {
		addressKey, ok := key.(ktypes.Address)
		if !ok {
			return false
		}
...
func (m *accountsMap) get(addr ktypes.Address) *account {
	a, ok := m.Load(addr)
	if !ok {
		return nil
	}
...
func (m *accountsMap) allTxs(includeEnqueued bool) ( // nolint
	allPromoted, allEnqueued map[ktypes.Address][]*ktypes.Interaction,
) {
	allPromoted = make(map[ktypes.Address][]*ktypes.Interaction)
	allEnqueued = make(map[ktypes.Address][]*ktypes.Interaction)

	m.Range(func(key, value interface{}) bool {
		addr, _ := key.(ktypes.Address)
		account := m.get(addr)
...
func (a *account) enqueue(tx *ktypes.Interaction) error {
...
// nextNonce.
func (a *account) promote() (uint64, []ktypes.Hash) {
	...
	promoted := uint64(0)
	promotedTxnHashes := make([]ktypes.Hash, 0)
