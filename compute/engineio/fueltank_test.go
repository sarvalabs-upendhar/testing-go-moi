...
func TestNewFuelTank(t *testing.T) {
	...
	tank := NewFuelTank(NewFuel(10))

	// Verify that the FuelTank was initialized with the correct capacity and consumed fuel
	require.Equal(t, NewFuel(0), tank.Consumed, "FuelTank was not initialized with 0 consumed fuel")
	require.Equal(t, NewFuel(10), tank.Capacity, "FuelTank was not initialized with the correct capacity")
}
...
func TestFuelTank_Level(t *testing.T) {
	...
	tank.Exhaust(NewFuel(5))
	require.Equal(t,
		new(big.Int).Sub(tank.Capacity, NewFuel(5)),
		tank.Level(),
...
func TestFuelTank_Exhaust(t *testing.T) {
	...
	require.True(t, tank.Exhaust(NewFuel(10)), "FuelTank did not exhaust all of its fuel")
	// Attempt to exhaust more fuel from the tank than is available and verify that the function returns false
	require.False(t, tank.Exhaust(NewFuel(1)), "FuelTank allowed more fuel to be exhausted than was available")
