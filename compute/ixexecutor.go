...
type IxExecutor struct {
	...
	snapshots map[common.Address]*state.Object
...
func (executor *IxExecutor) Execute(ixs common.Interactions, delta common.ContextDelta) error {
	...
	for _, ix := range executor.Interactions {
		...
		case common.IxValueTransfer:
			...
			for assetID, transferValue := range ix.TransferValues() {
				...
				if !tank.Exhaust(fuelConsumed) {
					return errors.Wrapf(common.ErrInsufficientFuel, "execution failed (%v)", ixtype)
				}
			}
		...
		case common.IxLogicInvoke:
			...
			// Exhaust fuel from tank
			if !tank.Exhaust(fuelConsumed) {
				return errors.Wrapf(common.ErrInvalidInteractionType, "execution failed (%v)", ixtype)
			}
...
func (executor *IxExecutor) CommitStateObjects(ix *common.Interaction) error {
	...
	if !accountRegistered {
		genesisHash, err := executor.getStateObject(common.SargaAddress).Commit()
		if err != nil {
			return err
		}
...
func (executor *IxExecutor) Revert() error {
	for _, ix := range executor.Interactions {
		...
		if !accountRegistered {
			if err := executor.state.Revert(executor.snapshots[common.SargaAddress]); err != nil {
				return err // This should not happen
			}
		}
	}
...
func (executor *IxExecutor) getReceipt(ix *common.Interaction) *common.Receipt {
...
func (executor *IxExecutor) updateSargaState(ix *common.Interaction) error {
	...
	sargaObject := executor.getStateObject(common.SargaAddress)
	if sargaObject == nil {
...
func (executor *IxExecutor) getStateObject(addr common.Address) *state.Object {
...
func (executor *IxExecutor) fetchStateObjects(ix *common.Interaction) error {
