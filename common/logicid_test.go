...
func TestLogicIDv0(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		...
		require.Equal(t, Hex2Bytes(string(logicID)), logicID.Bytes())
		...
		require.Equal(t, x.Persistent, identifier.PersistentState())
		require.Equal(t, x.Ephemeral, identifier.EphemeralState())
		require.Equal(t, x.Interactive, identifier.Interactive())
