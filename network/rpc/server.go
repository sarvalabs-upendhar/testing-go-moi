...
const (
	// ContextKeyRequestSender is default key for
	// RPC service function context to retrieve peer ID of current request sender
...
func (s *service) methodCall(ctx context.Context, sw *streamWrap, mtype *methodType, svcID ServiceID) error {
	...
	// 	if err != nil {
	// 		logrus.Info(" server : Failed to read from the stream : ", sw.stream.ID(), " err : ", err)
	// 		cancel()
	// 	}
	// }()
	...
	if errInter != nil {
		...
		if !ok {
			err = common.ErrInterfaceConversion
		}
	...
	return sendResponse(s.logger, sw, resp)
}
...
func (s *service) streamCall(ctx context.Context, sw *streamWrap, mtype *methodType, svcID ServiceID, size int) {
	...
	go func() {
		...
		for {
			...
			argv, err := readArgFromStream(sw, mtype.ArgType)
			if errors.Is(err, io.EOF) { // stream closed, no more to read.
				argsChan.Close()
	...
	go func() {
		...
		for {
			...
			err := sendResponse(s.logger, sw, Response{
				Service: svcID,
				Data:    v.Interface(),
			})
			if err != nil {
...
func sendResponse(logger hclog.Logger, s *streamWrap, resp Response) error {
...
func sendError(logger hclog.Logger, s *streamWrap, svcID ServiceID, err error) error {
	return sendResponse(logger, s, Response{
		Service: svcID,
		Error:   err.Error(),
...
func (server *Server) serverCall(call *Call) error {
	...
	if sh := server.statsHandler; sh != nil {
...
func isExported(name string) bool {
	nameRune, _ := utf8.DecodeRuneInString(name)

	return unicode.IsUpper(nameRune)
}
...
func (server *Server) register(rcvr interface{}, name string, useName bool) error {
	...
	if len(s.method) == 0 {
		...
		return errors.New("no exported methods")
	}
...
func suitableMethods(logger hclog.Logger, sname string, typ reflect.Type, reportErr bool) map[string]*methodType {
