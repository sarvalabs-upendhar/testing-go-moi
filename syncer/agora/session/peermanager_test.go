...
func TestUpdateFailedAttempts(t *testing.T) {
	...
	tt := []struct {
		name         string
		peers        map[identifiers.KramaID]int
		...
		},
		{
			...
			peers:        map[identifiers.KramaID]int{peerID[0]: 1},
			peerID:       peerID[0],
			delta:        1,
			isSuccess:    true,
			updatedCount: 2,
		},
		{
			...
			peers:        map[identifiers.KramaID]int{peerID[1]: 2},
			peerID:       peerID[1],
			delta:        -1,
			isSuccess:    true,
			updatedCount: 1,
...
func TestUpdatePeerStatus(t *testing.T) {
	...
	tt := []struct {
		name          string
		peers         map[identifiers.KramaID]bool
		peerID        identifiers.KramaID
		isSuccess     bool
		status        bool
		updatedStatus bool
		...
		},
		{
			...
			peers:         map[identifiers.KramaID]bool{peerID[0]: true},
			peerID:        peerID[0],
			isSuccess:     true,
			status:        false,
			updatedStatus: false,
		},
		{
			...
			peers:         map[identifiers.KramaID]bool{peerID[1]: false},
			peerID:        peerID[1],
			isSuccess:     true,
			status:        true,
			updatedStatus: true,
