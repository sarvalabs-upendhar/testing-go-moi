...
func TestUpdatePeerCount(t *testing.T) {
	testcases := []struct {
		name          string
		count         uint64
		testFn        func(pm *PersistenceManager)
		expectedCount uint64
	}{
		{
			name:          "Should create a new db entry",
			count:         5,
			expectedCount: 5,
		},
		{
			name:  "Should update existing db entry",
			count: 5,
			testFn: func(pm *PersistenceManager) {
				insertPeerCount(t, pm, 5)
			},
			expectedCount: 10,
		},
	}
	...
	for _, test := range testcases {
		t.Run(test.name, func(t *testing.T) {
			...
			err := pm.UpdatePeerCount(test.count)
			require.NoError(t, err)

			actualCount, err := pm.TotalPeersCount()
			require.NoError(t, err)
...
func TestGetPeerCount(t *testing.T) {
	testcases := []struct {
		...
		{
			name:          "Should return an error as there is no peer count in the db",
			expectedError: common.ErrKeyNotFound,
		},
		{
			name: "Should return the peer count successfully",
			testFn: func(pm *PersistenceManager) {
				insertPeerCount(t, pm, 5)
			},
			expectedCount: 5,
